function hex2b64(r) {
    ...
    if (t + 1 == r.length ? (a = parseInt(r.substring(t, t + 1), 16), n += b64map.charAt(a << 2)) : t + 2 == r.length && (a = parseInt(r.substring(t, t + 2), 16), n += b64map.charAt(a >> 2) + b64map.charAt((3 & a) << 4)), b64pad)
        for (;
...
function b64tohex(r) {
    var t, a, n, h = "",
        b = 0;
    for (t = 0; t < r.length && r.charAt(t) != b64pad; ++t) n = b64map.indexOf(r.charAt(t)), 0 > n || (0 == b ? (h += int2char(n >> 2), a = 3 & n, b = 1) : 1 == b ? (h += int2char(a << 2 | n >> 4), a = 15 & n, b = 2) : 2 == b ? (h += int2char(a), h += int2char(n >> 2), a = 3 & n, b = 3) : (h += int2char(a << 2 | n >> 4), h += int2char(15 & n), b = 0));
    return 1 == b && (h += int2char(a << 2)), h
}
...
function bnToString(t) {
    ...
    if (s-- > 0)
        for (h < this.DB && (r = this[s] >> h) > 0 && (n = !0, e = int2char(r)); s >= 0;) i > h ? (r = (this[s] & (1 << h) - 1) << i - h, r |= this[--s] >> (h += this.DB - i)) : (r = this[s] >> (h -= i) & o, 0 >= h && (h += this.DB, --s)), r > 0 && (n = !0), n && (e += int2char(r));
    return n ? e : "0"
}
        ...
        if (0 != f) {
            ...
            for (; --b >= 0;) {
                ...
                if ((r[v] += e.am(0, T, r, b, 0, a)) < T)
                    for (e.dlShiftTo(b, l), r.subTo(l, r); r[v] < --T;) r.subTo(l, r)
            }
...
function bnpExp(t, i) {
    ...
    var r = nbi(),
        ...
        e = nbits(t) - 1;
    for (n.copyTo(r); --e >= 0;)
        if (i.sqrTo(r, o), (t & 1 << e) > 0) i.mulTo(o, n, r);
        else {
            var s = r;
            r = o, o = s
        }
